...
export default function ConversationHistory({ sessionId }: ConversationHistoryProps) {
  ...
  if (!sessionId) {
    return (
      <div className="bg-gradient-to-br from-white to-amber-50 rounded-2xl shadow-xl p-8 text-center border border-amber-200">
        <MessageCircle className="w-16 h-16 text-amber-300 mx-auto mb-4" />
        <p className="text-stone-600">Start a call to see your conversation history</p>
      </div>
    );
  }

  return (
    <div className="bg-gradient-to-br from-white to-amber-50 rounded-2xl shadow-xl p-6 max-h-[600px] overflow-y-auto border border-amber-200">
      ...
      <div className="space-y-4">
        {messages.length === 0 ? (
          <p className="text-stone-400 text-center py-8">No messages yet</p>
        ) : (
              ...
              <div
                className={`max-w-[70%] rounded-2xl px-4 py-3 ${
                  message.role === 'user'
                    ...
                    : 'bg-amber-100 text-stone-800 rounded-bl-sm border border-amber-200'
                }`}
              >
